...
| 16 — Audit dan remediasi produksi | 🚧 Source selesai, validasi operator tertunda | Remediasi `planfixv2.md` M0–M10: SSRF, redaksi secret, enkripsi state Xray/CLI, cache policy, fixed-point UI, shutdown, TLS PostgreSQL, Caddy, dan hardening akses. Gate source/unit/frontend/build hijau. Release tetap **NO-GO** sampai rotasi/pencabutan kredensial, pembersihan riwayat, PostgreSQL TLS smoke test disposable, dan start-health-stop Xray dibuktikan. |
